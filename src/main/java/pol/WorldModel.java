...
import pol.DailyRoutines.RoutineType;
import pol.db.Cdf;
import pol.db.Column;
import pol.environment.*;
import pol.log.CdfFlatFormatterForRelation;
import pol.log.CdfMapper;
import pol.log.CdfMapperBuilder;
import pol.log.CdfSchemaFormatter;
import pol.log.CdfValueFormatter;
import pol.log.Characteristics;
import pol.log.DateTimeTypeAdapter;
import pol.log.ExtLogger;
import pol.log.GsonCsvSchemaFormatter;
import pol.log.GsonCsvValueFormatter;
import pol.log.GsonFormatter;
import pol.log.IterativeLogSchedule;
import pol.log.LocalDateTimeTypeAdapter;
import pol.log.LocalDateTypeAdapter;
import pol.log.LocalTimeTypeAdapter;
import pol.log.LogSchedule;
import pol.log.MasonGeometryTypeAdapter;
import pol.log.OutputFormatter;
import pol.log.ReferenceTypeAdapter;
import pol.log.ReflectionValueExtractor;
import pol.log.ReservedLogChannels;
import pol.log.Skip;
import pol.log.State;
import pol.log.SupplierExtractor;
import pol.utils.CollectionUtil;
import pol.utils.ColorUtils;
import pol.utils.DateTimeUtil;
import pol.utils.EventSchedule;
import pol.utils.Exclusion;
import pol.utils.GeoUtils;
import pol.utils.Manipulation;
import pol.utils.ManipulationLoader;
import pol.utils.MasterScheduler;
import pol.utils.SimulationEvent;
import pol.utils.SimulationTimeStepSetting;
import pol.utils.StringUtils;
...
public class WorldModel extends SimState {
	...
	public boolean spreadModelActive;
	...
	public WorldModel(long seed, WorldParameters params) throws IOException,
		...
		spreadModelActive = params.agentToAgentSpread || params.placeToAgentSpread;
		logger.info("Initialize Disease and Outlier");
		InfectiousModel.initializeGeneralParam(params);
		Outlier.initializeParameters(params);
		LoveNeed.initializeSpreadParam(params.agentToAgentSpread);
		AgentMobility.initializeSpreadParam(params.placeToAgentSpread);
	...
	public void nightlyRoutine() {
		...
		day = day + 1;
		InfectiousModel.incrementDaysSpreading();
		...
		for (Person person : this.agents.values()) {
			...
			// After Everything Done, update Outlier
			if (spreadModelActive) {
				person.getOutlier().update();
				person.checkAndWearMask();
			}
	...
	public List<Pub> getNearestPubs(MasonGeometry geom, int numberOfPubs) {
		...
		pubs.sort((Pub pub1, Pub pub2) -> {
			double distance1 = spatialNetwork.getDistance(geom, pub1.getLocation());
			double distance2 = spatialNetwork.getDistance(geom, pub2.getLocation());
			if (distance1 == distance2) {
				return Long.compare(pub1.getId(), pub2.getId());
			}
	...
	public Pub getAgentNearestPub(long agentId, int rank) {
		MasonGeometry geom = agents.get(agentId).getAgentLocation();
		List<Pub> pubs = getNearestPubs(geom, -1);
		return pubs.get(getSafeIndex(pubs.size(), rank));
	}

	public void infectRandomPub(String outlierType, double outlierDegree, double spreadDegree) {
		List<Pub> pubs = getUsablePubs();
		if (pubs == null)
			return;

		int randomPubIndex = random.nextInt(pubs.size());
		Pub randomPub = pubs.get(getSafeIndex(pubs.size(), randomPubIndex));
		randomPub.setSpreadDegree(spreadDegree);
		randomPub.setOutlierType(outlierType, outlierDegree);
		InfectiousModel.initCountOfDay();
		System.out.println("Random pub infected: " + randomPub.getId());
	}

	public void infectPopularPub(String outlierType, double outlierDegree, double spreadDegree, int rank) {
		List<Pub> pubs = getUsablePubs("numOfVisits");
		if (pubs == null)
			return;
		Pub pub = pubs.get(getSafeIndex(pubs.size(), rank));
		pub.setSpreadDegree(spreadDegree);
		pub.setOutlierType(outlierType, outlierDegree);
		InfectiousModel.initCountOfDay();
		System.out.println("Popular pub infected: " + pub.getId());
	}

	public static int getSafeIndex(int length, int index) {
		int safeIndex = 0;
		if (index < 0) {
	...
	public List<Pub> getUsablePubs(String sortBy) {
		List<Pub> useablePubs = getUsablePubs();

		if (sortBy.equals("numOfVisits")) {
			useablePubs.sort((Pub p1, Pub p2) -> {
				return Integer.compare(p2.getNumOfVisits(), p1.getNumOfVisits());
			});
		}
		return useablePubs;
	}
