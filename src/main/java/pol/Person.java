...
import pol.environment.Jitter;
import pol.environment.Job;
...
import pol.utils.InstructionQueue;
...
public class Person implements Steppable {
	...
	@State
	private InfectiousModel infectiousModel;
	...
	public Person(WorldModel model, long id, Family family) {
		...
		this.outlier = new Outlier(this);
		this.infectiousModel = model.spreadModelActive ? new InfectiousModel(this) : null;
	...
	public void setOutlierType(String outlierType) {
		this.outlier.setType(outlierType);
	}
	...
	public void setOutlierType(OutlierType outlierType, double outlierDegree) {
		this.outlier.setType(outlierType, outlierDegree);
	}

	public void setInitialOutlier(String strOutlierType) {
		this.infectiousModel.initialOutlier(OutlierType.valueOf(strOutlierType));
	}

	public void beenExposed(Person p) {
		this.infectiousModel.beenExposed(p);
	}

	public void beenExposed(BuildingUnit bu) {
		this.infectiousModel.beenExposed(bu);
	}

	public InfectiousStatus getInfectiousStatus() {
		return this.infectiousModel.getStatus();
	}

	public double getSpreadDegree() {
		return this.infectiousModel.getSpreadDegree();
	}

	public double getChanceBeInfected() {
		return this.infectiousModel.getChanceBeInfected();
	}

	public void backToNormal() {
		this.outlier.backToNormal();
	}

	public OutlierType getOutlierType() {
		return this.outlier.getType();
	}
	...
	public void step(SimState model) {
		...
		try {
			infectiousModel.nextStep(); // update infectious disease model
	...
	public DailyPlan planForSpecificDay(int dayDifferenceFromToday) {
		...
		if (this.isOutlierRightNow() && this.getOutlierType() == OutlierType.Work) {
		...
		else if (this.financialSafetyNeed.isEmployed()
	...
	public boolean isOutlierRightNow() {
		return this.outlier.isActiveRightNow();
	}
	...
	public InfectiousModel getInfectiousModel() {
		return this.infectiousModel;
	}

	public void checkAndWearMask() {
		this.infectiousModel.wearingMask();
	}

	public double getOutlierDegree() {
		return outlier.getOutlierDegree();
	}
	...
	public void setInterest(int interest) {
		this.interest = AgentInterest.valueOf(interest);
	}

	public void setInterest(double interest) {
		int interestInt = (int) Math.round(interest);
		this.interest = AgentInterest.valueOf(interestInt);
	}

	public void shiftInterest() {
		int curInterest = this.interest.getValue();
		this.interest = AgentInterest.valueOf(curInterest + 1);
	}

	public void shiftInterest(double amount) {
		int interestInt = (int) Math.round(this.interest.getValue() + amount);
		this.interest = AgentInterest.valueOf(interestInt);
	}
	...
	public void infectNearestPub(String outlierType, double outlierDegree, double spreadDegree, int rank) {
		Pub nearestPub = model.getAgentNearestPub(agentId, rank);
		System.out.println("Agent #" + agentId + " is infecting the nearestPub #" + nearestPub.getId());
		nearestPub.setSpreadDegree(spreadDegree);
		nearestPub.setOutlierType(outlierType, outlierDegree);
